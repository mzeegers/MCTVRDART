...
def objective_paramMC(x,W,p,lamb,epsilon):
    def objectivefun(param):
        f = 0

        plen = param.shape[0]//p.shape[0]
        #print(param.shape[0], x.shape, W.shape, p.shape, plen)

        for e in range(p.shape[0]):
            energyparam = param[e*plen:(e+1)*plen]
            [gv,K] = param2gv(energyparam)
            S = softseg(x[e],K,gv)
            if len(x[e].shape)==2:
                WS = W * S
            else:
            ...
            fit = np.sum(fit)
            tv = hubernorm(S,epsilon)
            f += fit + lamb*tv              ###SOM VAN DE NORMEN
        return f;
...
# objective function for reconstruction
def objective_recMC(param,W,p,lamb,epsilon,siz):
    def objectivefun(x):
        f = 0
        #print(siz)
        siz2 = tuple([p.shape[0]]+list(siz))
        #print(siz2)
        x = x.reshape(siz2)
        #print(x.shape)
...
def der_paramMC(x,W,p,lamb,epsilon):
    def derobjectivefun(param):
        
        gMC = np.zeros_like(param)
        ...
        for e in range(p.shape[0]): 
            ...
            for si in range(Ns):
                ...
                uo = logistic_np(x[e]-taoi,kg)
                xmtaoi = np.abs(x[e]-taoi)
                u = logistic_np(xmtaoi,np.abs(kg))
                u2 = u**2
                eMC = np.exp(-2*np.abs(kg)*xmtaoi)
                eu2 = eMC*u2
                
                if si == Ns-1:
                    Sdp = uo - eu2*Ki*(2*(x[e]-taoi)/Gdif+1)
                else:
                    taoip = th[si+1]
                    Kip = K[si+1]
                    Gdifp = gv[si+2] - gv[si+1]
                    kgp = Kip/Gdifp
                    uop = logistic_np(x[e]-taoip,kgp)
                    xmtaoip = np.abs(x[e]-taoip)
                    up = logistic_np(xmtaoip,np.abs(kgp))
                    up2 = up**2
                    ep = np.exp(-2*np.abs(kgp)*xmtaoip)
                    eup2 = ep*up2
                    Sdp = uo - eu2*Ki*(2*(x[e]-taoi)/Gdif+1) - uop - eup2*Ki*(1-2*(x[e]-taoip)/Gdifp)
                if len(x[e].shape)==2:
                    WSdp = W*Sdp
                else:
                    WSdp = FP3D(Sdp,W)
...
def der_recMC(param,W,p,lamb,epsilon,siz):
    def derobjectivefun(x):
        g = 0
        siz2 = tuple([p.shape[0]]+list(siz))
        ...
        x = x.reshape(siz2)
        Npixel = np.prod(siz)
        gMC = np.zeros(shape = (Npixel*p.shape[0]))
        plen = param.shape[0]//p.shape[0]
        for e in range(p.shape[0]):
            energyparam = param[e*plen:(e+1)*plen]
            [gv,K] = param2gv(energyparam)
...
# Joint estimation of reconstruction and parameters
def jointMC(W,p,x0, param0 ,lamb=1, Niter = 20, sf = 1, ch = 1):
    #print(W.shape, p.shape, x0.shape)
    #print("sf", sf)
    rec = x0
    param_esti = param0
    #print("param_esti", param_esti*sf)
    param_esti = estiparamMC(W,p, param_esti, rec, lamb)
    #print("param_esti2", param_esti*sf)
    #input()
    for iter in range(Niter):
        #print(str(param2gvMC(param_esti*sf, ch)))
        gvpre,Kpre = param2gv(param_esti)
        #print(gvpre,Kpre)
        Segrec,rec = reconMC(W,p, rec, param_esti, lamb)
        #print("param_esti3", param_esti*sf)
        #import pyqtgraph as pq
        #pq.image(rec*sf)
        #input()
        param_esti = estiparamMC(W,p, param_esti, rec, lamb)
        #print("param_esti4", param_esti*sf)
        gv,K = param2gv(param_esti)
        #print(gv, gvpre)
        #print("Diff", np.sum(np.abs(gv-gvpre))/np.sum(np.abs(gvpre)))
        if np.sum(np.abs(gv-gvpre))/np.sum(np.abs(gvpre))<1e-3:
            print('parameter estimation converged')
            break
        print(str(param2gvMC(param_esti*sf, ch)))
...
def reconMC(W,p,x0 ,param,lamb=1, Niter = 2, epsilon=1e-4, ch = 1):
    siz = x0.shape[1:]
    recMC = np.zeros_like(x0)
    segrecMC = np.zeros_like(x0)
    
    obj = objective_recMC(param,W,p,lamb,epsilon,siz)
    objder = der_recMC(param,W,p,lamb,epsilon,siz)
    res = minimize(obj, x0, method='L-BFGS-B', jac=objder, options={'maxiter': Niter,'disp': False})

    siz2 = x0.shape

    rec = res.x
    rec = rec.reshape(siz2)

    plen = param.shape[0]//p.shape[0]
    Segrec = np.zeros_like(rec)
    for e in range(p.shape[0]):
        energyparam = param[e*plen:(e+1)*plen]
        [gv,K] = param2gv(energyparam)
        Segrec[e,:,:] = softseg(rec[e],K,gv)

    recMC = rec
    SegrecMC = Segrec

    return SegrecMC,recMC;
...
def estiparamMC(W,p, param0, x, lamb=1, Niter = 5, epsilon=1e-4):
    #print("In EstiparamMC function", param0)
    #print(print("PARAM0", param0))
    objMC = objective_paramMC(x, W,p,lamb,epsilon)
    objderMC = der_paramMC(x,W,p,lamb,epsilon)
    
    res = minimize(objMC, param0, method='L-BFGS-B', jac=objderMC, options={'maxiter': Niter,'disp': False})
    #print("OBJECTIVE FUNCTION SUCCES")
    param = res.x
...
def param2gvMC(param, channels):
    N = int(len(param)/channels)
    for ch in range(0, channels):
        gvplus = param[N*ch:N*ch+int(N/2)]
        if(ch>0):
            gv = np.concatenate((gv, np.hstack((0,gvplus))))
            K = np.concatenate((K,param[N*ch+int(N/2):N*(ch+1)]))
        else:
            gv = np.hstack((0,gvplus))
            K = param[N*ch+int(N/2):N*(ch+1)]
    return gv,K;
