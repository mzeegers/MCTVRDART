...
os.makedirs('ParamSpaceImages/', exist_ok=True)

ParamsNoiseLevel = ['none', 'low', 'high']
ParamsAngles = [25,50,100,200]
ParamsNiterParam = [10,20,30]
ParamsNiterRecon = [2,50,100,200,300]

for NL in ParamsNoiseLevel:
    for A in ParamsAngles:
        for NiP in ParamsNiterParam:
            for NiR in ParamsNiterRecon:

                print("NL " + NL + "  A " + str(A) + "  NiP " +  str(NiP) + "  NiR " + str(NiR))

                f.write("NL " + NL + "  A " + str(A) + "  NiP " + str(NiP) + "  NiR " + str(NiR) + "\n")            
